...
@Component({
  selector: 'app-roles',
  template: `
    <app-settings-role-form *ngIf="(selectedRole || newRole) && rights"
    ...
    <button mat-flat-button color="primary" *ngIf="!newRole && !selectedRole" (click)="createRole()">Créer un Role</button>
    <app-search [query]="searchQuery"
                [searching]="loading"
                (search)="search($event)"
                placeholder="Rechercher un role"></app-search>
    <app-settings-name-list *ngIf="roles" [items]="roles"
...
export class SettingsRolesComponent implements OnInit {
  ...
  constructor(private roleService: RoleService, private infoService: InfoService, private jsonLdService: JsonLdService) {}
