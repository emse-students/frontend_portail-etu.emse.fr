...
@Component({
  ...
  template: `
    ...
    <button mat-flat-button color="primary" *ngIf="!newAsso && !selectedAsso" (click)="createAsso()">Créer une Liste</button>
    <app-search [query]="searchQuery"
                [searching]="loading"
                (search)="search($event)"
                placeholder="Rechercher une liste"></app-search>
    <app-settings-name-list *ngIf="assos" [items]="assos"
                            (deleted)="delete($event)"
                            (selected)="select($event)" [filter]="searchQuery" itemName="la liste">
...
export class SettingsListsComponent implements OnInit {
  ...
  delete(idAsso: number) {
    ...
    this.associationService.delete(idAsso).subscribe(
      () => {this.loading = false; this.infoService.pushSuccess('Liste supprimée avec succès'); },
      () => {this.loading = false; }
    );
  }
