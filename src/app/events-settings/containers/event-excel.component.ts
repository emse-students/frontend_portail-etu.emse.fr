...
@Component({
  ...
  template: `
    <div class="row justify-content-center" *ngIf="!loading">
      <button class="m-2" mat-flat-button color="primary" (click)="generate()">Générer l'excel des réservations</button>
    </div>
    <div class="row justify-content-center" *ngIf="loading">
...
export class EventExcelComponent implements OnInit {
  ...
  generate() {
    ...
    this.excelService.generateBookingExcel(this.event.id).subscribe(
      (ans) => {this.loading = false; window.open(environment.excel_url + '/' + ans.name); },
      () => {this.loading = false; }
    );
  }
