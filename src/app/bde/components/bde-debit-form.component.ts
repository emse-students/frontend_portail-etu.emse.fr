    ...
    <form [formGroup]="form" (ngSubmit)="submit()">
      ...
      <p>
        <mat-form-field class="w-100">
          ...
          <mat-error *ngIf="reason.touched && reason.invalid">{{getErrorMessage(reason)}}</mat-error>
        </mat-form-field>
      </p>
      ...
      <p class="loginButtons">
        <button class="ml-2" type="submit" mat-flat-button color="accent" [disabled]="form.invalid">Débiter</button>
      </p>
    </form>
  `,
  styles: []
...
export class BdeDebitFormComponent implements OnInit {
  ...
  positiveAccount(): ValidatorFn {
    return (control: AbstractControl): {[key: string]: any} | null => {
      if (
        this.form && this.user.value && this.user.value.balance && this.user.value.balance < this.amount.value
      ) {
