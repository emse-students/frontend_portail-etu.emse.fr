...
export class OperationService {
  ...
  public create(operation: NewOperation): Observable<Operation> {
    ...
    return this.http.post<Operation>(url, operation).pipe(tap(() => {this.userService.updateBalance(operation.amount); }));
  }

  public gets(): Observable<Operation[]> {
    const url = `${environment.api_url}/operations`;
    return this.http.get<Operation[]>(url).pipe(map((operations) => this.jsonLdService.parseCollection<Operation>(operations).collection));
  }
  ...
  public delete(operation: Operation): Observable<any> {
    ...
    return this.http.delete(url).pipe(tap(() => {this.userService.updateBalance(-operation.amount); }));
  }
}
